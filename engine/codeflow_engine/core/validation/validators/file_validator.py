...
class FileTypeValidator(BaseTypeValidator):
    ...
    def validate_file_upload(
        self, filename: str, content: bytes, max_size: int | None = None
    ) -> ValidationResult:
        ...
        if file_ext not in self.allowed_extensions:
            return ValidationResult.failure(
                f"File extension not allowed: {file_ext}", ValidationSeverity.HIGH
            )
    ...
    def _validate_text_content(self, content: bytes) -> ValidationResult:
        try:
            ...
            return ValidationResult.failure(
                "File contains invalid UTF-8 encoding", ValidationSeverity.HIGH
            )
