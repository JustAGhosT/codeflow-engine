    ...
    def validate(self, key: str, value: Any) -> ValidationResult:
        ...
        for obj_key, obj_value in value.items():
            if not self._is_safe_key(str(obj_key)):
                result.add_error(
                    f"Invalid nested key name: {key}.{obj_key}", ValidationSeverity.HIGH
                )
                continue
            nested_key = f"{key}.{obj_key}"
            obj_result = (
                self._value_validator(nested_key, obj_value)
                if self._value_validator
                else ValidationResult.success({"value": obj_value})
            )
            if not obj_result.is_valid:
                ...
                sanitized_object[obj_key] = self._unwrap_value(
                    obj_result.sanitized_data
                )
        if result.is_valid:
