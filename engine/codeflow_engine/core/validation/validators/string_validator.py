...
class StringTypeValidator(BaseTypeValidator):
    def __init__(self, max_length: int = 1000, security_patterns: SecurityPatterns | None = None) -> None:
    ...
    def _validate_format(self, key: str, value: str) -> ValidationResult:
        ...
        if "email" in key_lower and not self._is_valid_email(value):
            return ValidationResult.failure(f"Invalid email format in '{key}'", ValidationSeverity.MEDIUM)
        if "url" in key_lower and not self._is_valid_url(value):
            return ValidationResult.failure(f"Invalid URL format in '{key}'", ValidationSeverity.MEDIUM)
