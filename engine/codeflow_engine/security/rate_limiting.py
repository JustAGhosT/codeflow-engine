...
class RateLimiter:
    ...
    def is_allowed(
        self, key: str, limit: Optional[int] = None, tier: Optional[str] = None
        ...
        info = {
            ...
            "remaining": max(
                0, effective_limit - current_count - (1 if allowed else 0)
            ),
...
def rate_limit(
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if not allowed:
                # Rate limit exceeded
                _get_logger().warning("Rate limit exceeded", key=key, **info)
...
class RedisRateLimiter(RateLimiter):
    ...
    def __init__(self, redis_url: str, **kwargs):
        ...
        _get_logger().warning(
            "RedisRateLimiter not fully implemented, falling back to in-memory"
        )
